...
public class ProjectCustomRepositoryImpl implements ProjectCustomRepository{
    ...
    @Override
    public Page<ProjectDto> findProjectsByTitle(String searchValue, Pageable pageable) {
        List<ProjectDto> projects = queryFactory
                .selectFrom(projectSkill)
                .leftJoin(projectSkill.project, project)
                .leftJoin(projectSkill.skill, skill)
                .where(project.status.eq(ProjectStatus.ENROLLING).and(project.title.contains(searchValue)))
                .orderBy(project.registeredAt.desc()) //최신순이 맨 위에 오게
                .offset(pageable.getOffset())
                .limit(pageable.getPageSize())
                .transform(groupBy(project.id).list(
    ...
    @Override
    public Page<ProjectDto> findProjectsByNickName(String searchValue, Pageable pageable) {
        List<ProjectDto> projects = queryFactory
                .selectFrom(projectSkill)
                .leftJoin(projectSkill.project, project)
                .leftJoin(projectSkill.skill, skill)
                .where(project.status.eq(ProjectStatus.ENROLLING).and(project.registeredBy.eq(searchValue)))
                .orderBy(project.registeredAt.desc()) //최신순이 맨 위에 오게
                .offset(pageable.getOffset())
                .limit(pageable.getPageSize())
                .transform(groupBy(project.id).list(
    ...
    @Override
    public Page<ProjectDto> findProjectsBySkills(Collection<String> skills, Pageable pageable) {
        List<ProjectDto> projects = queryFactory
                .selectFrom(projectSkill)
                .leftJoin(projectSkill.project, project)
                .leftJoin(projectSkill.skill, skill)
                .where(project.status.eq(ProjectStatus.ENROLLING).and(skill.name.in(skills)))
                .orderBy(project.registeredAt.desc()) //최신순이 맨 위에 오게
                .offset(pageable.getOffset())
                .limit(pageable.getPageSize())
                .transform(groupBy(project.id).list(
